...
/*   Updated: 2023/03/22 18:53:02 by buozdemi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ClapTrap::takeDamage(unsigned int amount)
{
    if (this->hitpoints > 0)
    {
        std::cout << "ClapTrap " << this->name << " has been attacked!" << std::endl;
        this->hitpoints = this->hitpoints - amount;
    }
	else
        std::cout << "ClapTrap " << this->name << " can't take any more damage because it's dead." << std::endl;
}
...
void ClapTrap::beRepaired(unsigned int amount)
{
	if (this->hitpoints <= 0)
    {
        std::cout << "ClapTrap " << this->name << " cannot be repaired because it is not living." << std::endl;
        return ;
    }
    if (this->energypoints > 0)
    {
...
ClapTrap& ClapTrap::operator=(const ClapTrap& c)
{
	if (this != &c)
	{
	    this->setAttackdamage(c.attackdamage);
	    this->setEnergypoints(c.energypoints);
	    this->setHitpoints(c.hitpoints);
	    this->setName(c.name);
	}
    return (*this);
}
